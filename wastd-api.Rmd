...
Note this link currently points to a development version of WAStD, as the 
[public API](https://strandings-test.dpaw.wa.gov.au/api/1/animal-encounters/) lies
behind an authentication firewall. Authenticated users however can download the 
CSV version through the web version (link above > Get > CSV), then `read.csv()`
that file here.
