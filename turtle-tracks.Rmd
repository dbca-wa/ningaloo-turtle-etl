...
[Google Fusion Table](https://fusiontables.google.com/DataSource?docid=1wL_dSRNuUCyukJjiUo8RDvFQ0ejWoRpJo2p3S5Rm#map:id=6).

This data view shows one row per track or nest count and each related damage observation.
If a nest has several damage observations, the track or nest details are repeated.


```{r loaddata_csv, eval=F}
d_tbl <- tbl_df(read.csv("data/TrackCount 0.10View.csv", header=T, as.is=T))
```

## Using live data from Google Fusion Tables
To use real-time data, load the data through the Google Fusion Tables API v2
...
grouped by species, and facet-wrapped by location.
...
```{r plot_tally, message=F, warning=F}
ggplot(tally_data, aes(x=observation_date, y=n, colour=species)) +
  geom_point() + geom_line() + facet_wrap(c("location", "species"))
```
...
```{r plot_tally_fresh_thv, message=F}
ggplot(filter(tally_fresh, location=="Thevenard Is"), aes(x=observation_date, y=n, colour=nest_type)) +
  geom_point() + geom_line() + facet_wrap(~species, ncol=2) + ggtitle("Thevenard Island Turtle Tracks")
```
