...
sites <- con$tblSections %>%
  transmute(
    ...
    ymax=-as.numeric(SubSect.NE.lat),
    ymin=-as.numeric(SubSect.SW.lat),
    xmax=as.numeric(SubSect.NE.long),
    xmin=as.numeric(SubSect.SW.long))
...
sites[which(sites$subsection=="Red Bluff"),]$xmax <- 113.458
sites[which(sites$subsection=="Red Bluff"),]$ymax <- -24.016
...
coords <- sites %>%
  transmute(
    sw.lon=xmin, sw.lat=ymin,
    se.lon=xmax, se.lat=ymin,
    ne.lon=xmax, ne.lat=ymax,
    nw.lon=xmin, nw.lat=ymax,
    end.lon=xmin, end.lat=ymin) %>%
  as.matrix()
```
...
surveys <- con$tblDBAreaSurveyed %>%
  ...
  mutate(
    date.id=parse_date_time(date.id, orders=ord, tz=tz),
    time.HT=parse_date_time(time.HT, orders=ord, tz=tz)) %>%
  mutate(
  ...
  select(
    -starts_with("Ysn.id"), -starts_with("sw"), -starts_with("se"), 
    -starts_with("nw"), -starts_with("ne")) %>%
  select(survey_id, date, division, section, subsection, everything())
...
* Count the observations of each group (= total number of nests of species x seen on date y at subsection z)
* Append survey level data
