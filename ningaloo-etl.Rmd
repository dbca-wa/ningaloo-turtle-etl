---
title: "Ningaloo Turtle Program Data ETL"
...
accessible formats. Data are read from a snapshot on the internal data catalogue,
processed locally, and uploaded as various outputs back to the data catalogue.

**Note** The data shown are not quality controlled, and therefore could contain
duplication or gaps. This notebook currently is a work in progress.
...
nests <- con$tblDBNestingSurvey %>%
  ...
  select(-nest_type_id, -confidence_id, -position_id, -yes_no_id, -species_id)
...
and species.
```{r}
new_nests_by_survey <- tbl_df(nests) %>%
  filter(nest_type=="New") %>%
  group_by(subsection, date, species_name) %>%
  tally(sort=T) %>%
  ungroup() %>%
  left_join(surveys)
write.csv(new_nests_by_survey, file = "data/new_nests_by_survey.csv")
DT::datatable(new_nests_by_survey)
```
...
and species.
```{r}
false_crawls_by_survey <- tbl_df(crawls) %>%
  group_by(subsection, date, species_name) %>%
  tally(sort=T) %>%
  ungroup() %>%
  left_join(surveys)
write.csv(false_crawls_by_survey, file = "data/false_crawls_by_survey.csv")
DT::datatable(false_crawls_by_survey)
```
